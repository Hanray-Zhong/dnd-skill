...
`ability-modifier` 来自《玩家手册》第 173 页的“属性值减 10 后除以 2”和第 7 页的向下取整规则。响应保留公式目录身份、适用条件、输入、修正策略与优先级、逐步运算和带单位结果；状态写入边界会独立复算后再提交。当前纵向切片尚无权威角色构成、桌规或模组效果记录可作为修正来源，因此只执行空修正集；任意非空修正请求都会在完整同级冲突检查后因来源未获权威状态授权而拒绝。该纵向切片只保存已确认名册中角色的一项派生数据，不创建角色构成，也不扩大到尚未实现的完整角色生命周期。
