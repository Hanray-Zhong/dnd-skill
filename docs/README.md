...
Issue #2、#3、#8 与 #9 的纵向切片已经建立十一项 Skill 目录、`dnd-5e` 统一进程门面、固定战役工作区、SQLite 权威状态库边界、Session Zero 完成事务及运行期消息入口。当前可安全创建、配置和重新打开战役，在玩家与安全确认齐全、角色控制关系无冲突时原子写入开团状态、初始受众与 `table` 权威共享场景，并可按名册、场景参与者、交互模式和角色控制权分类 IC、行动、内心、OOC 与待验证系统消息；需要保留的角色消息经幂等状态事务记录，普通 OOC、系统消息文本与拒绝输入不推进修订。Issue #4 增加了完整三宝书开发构建、固定 Markdown 资产查询、本地预览打包和授权状态门禁；Issue #5 在该资产上增加了规则实体查询，以及由构建期已复核、有界正文证据支持的具体实体优先决策记录。运行方式见仓库根目录的 [README.md](../README.md)。
...
[GitHub Issue #5](https://github.com/Hanray-Zhong/dnd-skill/issues/5) 的交付范围是按名称或稳定标识读取代表实体，并在实体与一般默认规则存在可追溯引用时输出具体优先的决策记录；它不提前实现角色、战斗、奖励或公式行为。边界见[来源到行为覆盖矩阵](dnd-5e-source-to-behavior-coverage-matrix.md#issue-5-交付边界)。
