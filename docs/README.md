...
Issue #2、#3、#8 与 #9 的纵向切片已经建立十一项 Skill 目录、`dnd-5e` 统一进程门面、固定战役工作区、SQLite 权威状态库边界、Session Zero 完成事务及运行期消息入口。当前可安全创建、配置和重新打开战役，在玩家与安全确认齐全、角色控制关系无冲突时原子写入开团状态与初始受众，并可按名册和角色控制权分类 IC、行动、内心、OOC 与可信系统消息；需要保留的游戏相关消息经幂等状态事务记录，普通 OOC 与拒绝输入不推进修订。Issue #4 增加了完整三宝书开发构建、固定 Markdown 资产查询、本地预览打包和授权状态门禁；Issue #5 在该资产上增加了规则实体查询，以及由构建期已复核、有界正文证据支持的具体实体优先决策记录。运行方式见仓库根目录的 [README.md](../README.md)。
