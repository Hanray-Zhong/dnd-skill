...
`recalculate` 当前提供 Issue #6 的代表性公式 `ability-modifier`。输入必须显式携带 `ability_score` 单位；公式目录声明适用条件、允许的修正操作和完整规则优先级。当前版本尚无权威角色构成、桌规或模组效果状态可解析修正来源，因此 `--modifiers` 的可执行值仅为 `[]`：非空请求会先检查单位与所有优先级层的同级冲突，再以未授权来源拒绝，不能凭调用方自报的 `approved_table_rule` 等字符串获得优先权。成功结果记录公式目录版本与哈希、适用条件、两条规则来源、输入、空修正集、减法、除法和向下取整步骤，再由唯一状态边界复算后原子保存为角色派生数据。相同幂等请求返回原事务，重新打开战役可恢复同一审计轨迹。缺失输入、单位冲突、越界输入或同优先级覆盖冲突会在写入前拒绝。完整角色构成与可信修正来源仍由后续角色能力负责，本入口不把临时输入冒充角色构成。
