...
公式重算只接受公式目录声明的适用条件、输入、单位、修正操作和优先级。当前没有权威角色构成、桌规或模组效果状态时只执行空修正集；非空修正必须先检查所有优先级层的同级冲突，再因缺少权威来源授权而拒绝，不能信任调用方自报的优先级。必须把缺失输入、单位不一致、同优先级冲突或目录身份错误作为拒绝返回，不得临时心算后绕过审计轨迹或状态事务。
